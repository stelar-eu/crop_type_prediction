...
class ConvLSTM(pl.LightningModule):
    def __init__(self, run_path, linear_encoder, learning_rate=1e-3, parcel_loss=False,
        ...
        self.clstm_1e = CLSTM_cell(shape=(366, 366), input_channels=16, filter_size=5, num_features=64)
        ...
        self.clstm_2e = CLSTM_cell(shape=(183, 183), input_channels=64, filter_size=5, num_features=96)
        ...
        self.clstm_3e = CLSTM_cell(shape=(92, 92), input_channels=96, filter_size=5, num_features=96)

        # Decoder
        # -------
        #self.clstm_1d = CLSTM_cell(shape=(16, 16), input_channels=96, filter_size=5, num_features=96)
        self.clstm_1d = CLSTM_cell(shape=(92, 92), input_channels=96, filter_size=5, num_features=96)
        ...
        self.clstm_2d = CLSTM_cell(shape=(183, 183), input_channels=96, filter_size=5, num_features=96)
        ...
        self.clstm_3d = CLSTM_cell(shape=(366, 366), input_channels=96, filter_size=5, num_features=64)
    ...
    def forward(self, input):
        ...
        #print(" after transpose input.shape", input.shape)
        hidden_states = []
    ...
    def training_step(self, batch, batch_idx):
        ...
        #find the list of all unique values in label

        # Flatten the label tensor
        flat_label = label.view(-1)

        # Find unique values
        unique_values = torch.unique(flat_label)

        # Convert to a Python list
        unique_values_list = unique_values.tolist()

        #print("List of unique values in label:", unique_values_list)

        #print("what here : inputs.shape, label.shape ", inputs.shape, label.shape)

        pred = self(inputs)  # (B, K, H, W)
        #print("pred.shape input to loss", pred.shape)
        if self.parcel_loss:
            ...
            #print("label.shape input to loss", label.shape)
            loss = self.lossfunction(pred, label)
            #print("Or going for the else ? ")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.parcel_loss:
            ...
            parcels_K = parcels[:, None, :, :].repeat(1, pred.size(1), 1, 1)  # (B, K, H, W)
            #parcels_K = parcels[:].repeat(1, pred.size(1), 1, 1)  # (B, K, H, W)
            #print("parcels_K.shape", parcels_K.shape)
            # Note: a new masked array must be created in order to avoid inplace
            # operations on the label/pred variables. Otherwise the optimizer
            # will throw an error because it requires the variables to be unchanged
            # for gradient computation
            ...
            mask = (parcels) & (label != 0)
            mask_K = (parcels_K) & (label[:, None, :, :].repeat(1, pred.size(1), 1, 1) != 0)
            #mask_K = (parcels_K) & (label[:].repeat(1, pred.size(1), 1, 1) != 0)
            #print("mask_K.shape", mask_K.shape)
