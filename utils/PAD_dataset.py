...
class PADDataset(Dataset):
    ...
    def __init__(
        ...
        self.num_buckets = len(pd.date_range(start=f'2020-01-01', end=f'2020-07-01', freq=self.group_freq)) - 1

        self.saved_medians = saved_medians

        #print("prefix", prefix)
        #print("group_freq", group_freq)
        ...
        #print("Also here ? self.medians_dir", self.medians_dir)
    ...
    def load_medians(self, path: Path, subpatch_id: int, start_bin: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if self.fixed_window:
            #print("self.num_bands", self.num_bands)
            #print("self.output_size[0]", self.output_size[0])
            #print("self.output_size[1]", self.output_size[1])
            medians = np.empty((6, self.num_bands, self.output_size[0], self.output_size[1]),
                                dtype=self.medians_dtype)
            #print("Is it here coming here ?")
            #print("medians.shape", medians.shape)
            #print("len(medians)", len(medians))
            #print("medians[0].shape", medians[0].shape)

        else:
            medians = np.empty((self.window_len, self.num_bands, self.output_size[0], self.output_size[1]),
                                dtype=self.medians_dtype)
            print("Or is it instead coming here ?")

        #print("subpatch_id", subpatch_id)
        #print("self.num_subpatches", self.num_subpatches)
        #padded_id = f'{str(subpatch_id).rjust(len(str(self.num_subpatches)), "0")}'
        #padded_id = f'{str(subpatch_id).zfill(len(str(self.num_subpatches)))}' # this is the original line of code

        padded_id = str(f'{str(subpatch_id).rjust(len(str(self.num_subpatches)), "0")}').zfill(2)

        #print("padded_id", padded_id)
        #print("path", path)
        #path = '/home/luser/S4A-Models/logs/medians'
        #print("f'sub}_bin*'", f'sub{padded_id}_bin*')
        #lsited_files = path.glob(f'sub{padded_id}_bin*').item()
        #print("lsited_files", lsited_files)
        #print("why isnt the below below line executing ? ")
        #test_it = path.glob(f'sub{padded_id}_bin*')
        #print(test_it)
        median_files = sorted(path.glob(f'sub{padded_id}_bin*')) # here is the problem
        #print("len(median_files)", len(median_files))
        #median_files = sorted(path.glob(f'sub*'))
        #print("why isnt the below line executing ? ")
        #print("median_files", median_files)
        #median_files = sorted(list(path.glob(f'sub{padded_id}_bin*')))

        #import glob
        #median_files = glob.glob(path)

        #median_files = sorted(list(path.glob(f'sub'+str(padded_id)+'_bin*.npy')))
        ...
        for i, bin_idx in enumerate(range(start_month, end_month)):
            #print("Length of median_files:", len(median_files))
            #print("bin_idx:", bin_idx)
            #print("median_files[]", median_files)
            median = np.load(median_files[bin_idx]).astype(self.medians_dtype)
            #print("median.shape", median.shape)
            #print("medians[i].shape", medians[i].shape)
            medians[i] = median.copy()

        # Read labels
        #padded_id = str(f'{str(subpatch_id).rjust(len(str(self.num_subpatches)), "0")}').zfill(6)
        padded_id = f'{str(subpatch_id).rjust(len(str(self.num_subpatches)), "0")}'
        #print("path in dataset class", path)
        labels = np.load(path / f'labels_sub{padded_id}.npy').astype(self.label_dtype)
        #print("labels.shape from dataset PAD dataset class", labels.shape)
        #print("just labels", labels)
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if self.saved_medians:
            ...
            #print("Is it here ? self.medians_dir", self.medians_dir)
            ...
            #print("medians, labels", medians, labels)
        else:
