# to run this file : python3 export_medians_multi.py --prefix_coco 20231218125941

# to run this file : python3 export_medians_multi.py --prefix_coco 20240220154731

# 20240220154731

# FileNotFoundError: [Errno 2] No such file or directory: b'coco_files/netcdf/2019_31TCG_patch_13_04.nc'
...
def process_patch(out_path, mode, num_buckets, root_chdata_path, bands, padded_patch_height,
    ...
    # Calculate medians
    #print("root_coco_path", root_coco_path)
    #print("patch_info", patch_info)

    #print(f'Attempting to open file: {root_coco_path / patch_info["file_name"]}')
    #netcdf = netCDF4.Dataset(root_coco_path / patch_info['file_name'], 'r')

    netcdf = netCDF4.Dataset(root_chdata_path / patch_info['file_name'], 'r')

    '''try:
        netcdf = netCDF4.Dataset(root_chdata_path / patch_info['file_name'], 'r')
    except FileNotFoundError:
        #print(f'File not found: {root_chdata_path / patch_info["file_name"]}. Skipping...')
        return'''
    ...
    for i in range(medians.shape[0]):
        for j in range(medians.shape[1]):
            for t in range(num_bins):
                #print("medians[t].shape", medians[t].shape)
                #print("medians[i, j, t].shape", medians[i, j, t].shape)
                #break
                #print("medians[i, j, t, :, :, :].astype(medians_dtype)", medians[i, j, t].astype(medians_dtype).shape)
                #np.save(patch_dir / f'sub{str(sub_idx).rjust(subs_pad, "0")}_bin{str(t).rjust(bins_pad, "0")}', medians[i, j, t, :, :, :].astype(medians_dtype))
                #np.save(patch_dir / f'sub{str(sub_idx).rjust(subs_pad, "0")}_bin{str(t).rjust(bins_pad, "0")}', medians[i, j, t].astype(medians_dtype))
                np.save(patch_dir / f'sub{str(sub_idx).rjust(subs_pad, "0")}_bin{str(t).rjust(bins_pad, "0")}', medians[t].astype(medians_dtype))
                #print("any saves")
            sub_idx += 1
    ...
    #print("WHat is this first labels.shape", labels.shape)

    #labels = labels.squeeze()  # shape: (subpatches_in_row, subpatches_in_col, height, width)
    
    #print("WHat is this labels.shape", labels.shape)

    lbl_idx = 0
    lbl_pad = len(str(labels.shape[0] * labels.shape[1]))

    #print("lbl_pad", lbl_pad)
    
    for i in range(labels.shape[0]):
        for j in range(labels.shape[1]):
            #print("here i", i)
            #print("here j", j)
            #print(" whats is being saveed labels.shape", labels.shape)
            #print(" And here labels.shape", labels[i, j].shape)
            #np.save(patch_dir / f'labels_sub{str(lbl_idx).rjust(lbl_pad, "0")}', labels[i, j, :, :].astype(label_dtype))
            np.save(patch_dir / f'labels_sub{str(lbl_idx).rjust(lbl_pad, "0")}', labels[i, j].astype(label_dtype))
            #np.save(patch_dir / f'labels_sub{str(lbl_idx).rjust(lbl_pad, "0")}', labels.astype(label_dtype))
            lbl_idx += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--root_chdata_path', type=str, default='dataset/', required=False,
                        help='Root path for coco file. Default "dataset/".')
    #parser.add_argument('--prefix_coco', type=str, default=None, required=False,
    parser.add_argument('--prefix_coco', type=str, default='exp1_patches5000_strat', required=False,
    ...
    root_chdata_path = Path(args.root_chdata_path)
    #print("root_coco_path", root_coco_path)
    
    medians_dtype = np.float32
    label_dtype = np.int16

    #print("args.bands", args.bands)
    ...
    num_buckets = len(pd.date_range(start=f'2020-01-01', end=f'2020-07-01', freq=args.group_freq)) - 1 # num_buckets represents the number of time intervals or buckets between '2020-01-01' and '2021-01-01' with a frequency specified by args.group_freq
    ...
    #print("padded_patch_height, padded_patch_width, pad_top, pad_bot, pad_left, pad_right", padded_patch_height, padded_patch_width, pad_top, pad_bot, pad_left, pad_right)
    ...
    for mode in ['train', 'val', 'test']:
        ...
        func = partial(process_patch, out_path, mode, num_buckets, root_chdata_path,
